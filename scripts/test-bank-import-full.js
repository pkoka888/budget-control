...
(async () => {
  ...
  try {
    ...
    if (apiResponse) {
      console.log('\n✅ Import completed!');
      console.log(`   - Success: ${apiResponse.success ?? 0} transactions`);
      console.log(`   - Failed: ${apiResponse.failed ?? 0} files`);
      if (apiResponse.files && apiResponse.files.length > 0) {
        console.log(`   - Files processed: ${apiResponse.files.length}`);
        apiResponse.files.forEach(file => {
          if (file.status === 'success') {
            console.log(`     ✓ ${file.name}: ${file.imported} imported, ${file.skipped} skipped`);
          } else {
